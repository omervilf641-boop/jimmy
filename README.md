...
**חלון שחור?** אם ג'רוויס נפתח כמלבן ריק, הרנדרר כנראה מצייר בסדר גמור והמסך לא מקבל
את התמונה — תקלת קומפוזיטור, בדרך כלל אחרי עדכון דרייבר. לבדוק בשנייה:

```bash
npm start -- --no-gpu
```

אם הממשק מופיע — זו הייתה הסיבה. אפשר לקבע עם `JARVIS_NO_GPU=1`. לאבחון מלא,
כולל צילום של מה שהחלון באמת מצייר בפנים: `npx electron diagnose.js`.
